...
internal sealed class ScriptKeywordProcessor : UnityEditor.AssetModificationProcessor {
    ...
    private const string company1 = "Crenix Games";
    private const string websiteUrl1 = " - http://crenixgames.com";
    ...
    public static void OnWillCreateAsset(string path) {
        ...
        string userName = userInfoType.GetProperty("displayName").GetValue(userInfo, null) as string;
        string userEmail = userInfoType.GetProperty("userName").GetValue(userInfo, null) as string;

        string nameSpace = string.Empty;
        string[] nameSpaceWords = path.Replace(".cs", string.Empty).Replace('/', '.').Split('.');

        for(int i = 0; i < nameSpaceWords.Length; i++) {
            if(nameSpaceWords[i].ToLower().Contains("scripts") || nameSpaceWords[i].ToLower().Contains("script")) {
                i++;
                for(;i < nameSpaceWords.Length-1; i++) {
                    nameSpace += nameSpaceWords[i];
                    if(i < nameSpaceWords.Length - 2) {
                        nameSpace += '.';
                    }
                }
                file = file.Replace("#FOLDERNAMESPACE#","namespace " + nameSpace + "{");
                file = file.Replace("#SPACE#", "    ");
                file = file.Replace("#ENDNAMESPACE#", "}");
            }
        }
        ...
        file = file.Replace("#DEVELOPERNAME#", userName + " - " + userEmail);
